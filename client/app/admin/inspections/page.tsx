...
export default function Page() {
  ...
  const [resultDialog, setResultDialog] = useState({ open: false, message: "", success: true });
  ...
  const submitAssignment = async () => {
    try {
      ...
      if (res.ok) {
        setResultDialog({ open: true, message: data.message, success: true });
        setForm({ inspectorId: "", pipelineId: "", segmentId: "", inspectionDate: "" });
      } else {
        setResultDialog({ open: true, message: data.message || "Failed to assign inspection", success: false });
      }
    } catch (err) {
      setResultDialog({ open: true, message: "Network error", success: false });
    } finally {
  ...
  const handleRowClick = (type: TableType, item: any) => {
    ...
    if (selectedId !== undefined) {
      setForm(prev => ({ ...prev, [`${type}Id`]: selectedId.toString() }));
    }
  };
  ...
  const renderTable = (type: TableType) => {
    ...
    if (type === "inspector") {
      data = inspectors.map(i => ({ InspectorID: i.InspectorID, Name: i.Name, Phone: i.Phone, Email: i.Email }));
    }
      ...
      <Table className="mt-4">
        ...
        <TableBody>
          {data.map((item, index) => (
            ...
            >
              {Object.values(item).map((val, i) => (
                <TableCell key={i}>{typeof val === "string" || typeof val === "number" ? val : JSON.stringify(val)}</TableCell>
              ))}
            </TableRow>
          ))}
        </TableBody>
    ...
    <SidebarProvider>
      ...
      <SidebarInset>
        ...
        <div className="flex flex-1 flex-col p-4">
          ...
          <form onSubmit={(e) => { e.preventDefault(); setConfirmOpen(true); }} className="space-y-4 max-w-xl">
            <div>
              <Label>Inspector ID</Label>
              <Input name="inspectorId" value={form.inspectorId} onChange={handleChange} />
            </div>
      ...
      <AlertDialog open={confirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Assign this inspection?</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to assign this inspection to Inspector ID {form.inspectorId}?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setConfirmOpen(false)}>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={submitAssignment}>Confirm</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
